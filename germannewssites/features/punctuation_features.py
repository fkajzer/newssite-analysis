...
class PunctuationFeatures(BaseEstimator):
    ...
    def transform(self, documents):
        ...
        tokens_list = self.create_token_lemma_list(documents)
    ...
    def create_token_lemma_list(self, documents):
        token_list = []
        for doc in documents:
            doc_token = []
            for index in doc:
                for token in doc[index]['token']:
                    doc_token.append(token['lemma'])
            token_list.append(doc_token)
